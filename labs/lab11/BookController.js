...
var BookController = /** @class */ (function () {
    ...
    // Get all books
    BookController.prototype.getAllBooks = function () {
        return __spreadArray([], this.books, true); // Return a copy to prevent direct modification
    };
    // Get book count
    BookController.prototype.getBookCount = function () {
        return this.books.length;
    };
    // Print all books
    BookController.prototype.printBookList = function () {
        if (this.books.length === 0) {
            console.log("The book list is empty.");
            return;
        }
        this.books.forEach(function (book, index) {
            console.log("Book 0".concat((index + 1).toString(), ": ").concat(book.getTitle()));
            console.log("\tISBN: ".concat(book.getISBN()));
            console.log("\tauthor: ".concat(book.getAuthor()));
            console.log("\tyear: ".concat(book.getYear()));
            console.log("");
        });
    };
